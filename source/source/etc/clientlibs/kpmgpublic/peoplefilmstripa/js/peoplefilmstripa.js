...
define([
    ...
    function($, $clamp, Tracking) {
        ...
        var Peoplefilmstripa = function(elem) {
            element = elem;
            $(window).on('resize', initSly);
            init();
            $(document).on('reflow.complete', handleReflow);
            var trck = new Tracking(elem, 'Peoplefilmstripa');
        ...
        function init() {
            var $headerClamp = $('.item-header', element),
                ...
                $descClamp = $('div.pull-left div', element),
            ...
            $headerClamp.each(function(index, element) {
                if (!is_chrome && $(this).html().length < 32) {
                    $(this).removeClass('line-clamp');
                }
            });
...
*/
        ...
        function initSly(evt) {
            hideScroll = function() {
                var $leftControl = $('.controls .prevPage', element),
                    ...
                    totalSlideWidth = ($(".each-slide", element).width() + 7) * ($(".each-slide", element).length);
        ...
        function accessibiltyFeatures() {
            $(".peoplefilmstripa-tab-entry", element).on('focus', function(e) {
                e.preventDefault();
            ...
            $(".each-slide", element).on('keydown', function(e) {
                ...
                if (e.shiftKey && e.which === 9) {
                    ...
                    $(".peoplefilmstripa-tab-entry", element).parents(".row-same-height").prev().find("li:last").attr("tabindex", "-1").addClass("focusOutline").find("a").trigger('focus');
                } else if (e.which === 9) {
                    _this.removeClass("focusOutline").removeAttr("tabindex");
