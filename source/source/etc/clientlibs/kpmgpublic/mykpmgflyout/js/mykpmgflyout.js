    ...
    function($, $clamp, Tracking, Handlebars, owl, Helpers, Handlebarshelpers, personalizationUtils, genericErrorDialog) {
        ...
        var Mykpmgflyout = function(elem, componentName) {
            ...
            $(document).ready(function() {
                ...
                setTimeout(function() {
                    $('.sidr a[href*="/home/user/overlays/kpmglearnmore"]').on('click', learnMoreModalTriggerFn);
                }, 0);
            });
            //
            Mykpmgflyout.learnMoreModalTriggerFn = learnMoreModalTriggerFn;
            //
            function learnMoreModalTriggerFn(e) {
                e.preventDefault();
            ...
            if (personalizationUtils.accountUtils.isLoggedIn()) {
                ...
                // $(".rsOppModal button.ignoreandlogout").on('click', function(event){
                //
                ...
                // });
                //
                $(".genericOppModal button.continueButton").on('click', function(event){

                    // event.preventDefault();
                    // var homeURL = window.location.pathname.split('/').slice(0, (window.kpmgPersonalize.misc.isAuthor ? 3 : 1));
                    // homeURL.push(localeObj.countryCode, localeObj.languageCode, "home/dashboard/myaccount.html#communication-preferences");
                    // tracking RS OPP acceptance
                    window.digitalData.page.pageInfo.modalType = "RS Generic OPP Modal";
                    tracking.track('PrivacyAccept_Subscriptions', '');
                    //window.location = homeURL.join("/");
                });
                personalizationUtils.myKpmgFlyoutUtils.getLinksJson()
                    .done(function (data) {
                        var order = ['dashboard', 'profile', 'subscriptionsLink', 'interests', 'library', 'logout'];
            ...
            //Account flyout subscription bizarre issue with old header
            if ($(".logged-in-list").length > 0) {
                var subscriptionUrl = $('.logged-in-list li:nth-child(3)').find("a").data("uri");
                if (subscriptionUrl !== undefined) {
                    $('.logged-in-list li:nth-child(3)').find("a").attr("href",subscriptionUrl);
                }
            }            
